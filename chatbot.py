...
if __name__ == "__main__":
    ...
    with st.sidebar:
        ...
        option = st.selectbox(
            "Search and select a website:",
            ["Select an option"] + list(links.keys())  # Adding default option
        )

        # Open link when a valid option is selected
        if option in links:
    ...
    st.header("Vidya Chatbot")
    prompt = st.text_area("**Ask a question related to College:**")
    ask = st.button(
        "🔥 Ask",
    )
